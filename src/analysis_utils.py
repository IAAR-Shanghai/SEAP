...
def lm_eval_results_to_df(src):
    ...
    for task, res in data["results"].items():
        ...
        for key, val in res.items():
            ...
            rows.append(dict(Task=task, Version=ver, Filter=flt, Shot=shot, Metric=metric,
...
def normalize_metric(df: pd.DataFrame) -> pd.DataFrame:
    """
    添加列 'Metric_for_use'，用于统一各任务的主评估指标。
    """
    df = df.copy()
...
def build_top_configs(df: pd.DataFrame, top_k: int = 5) -> pd.DataFrame:
    """
    获取每个 TaskGroup + prompt 下得分最高的 top_k 校准任务，并计算采样权重。
    """
    # 映射任务到组
...
def plot_prompt_heatmap(df, prompt_type="knowledge", normalize_mode="best", task_groups=None, figsize=(8, 6), model_name=None):
    task_display = {
        "gsm8k": "GSM8K", "mathqa": "MathQA", "arc_challenge": "ARC-C", "arc_easy": "ARC-E", "openbookqa": "OBQA",
        "hellaswag": "HellaSwag", "winogrande": "WinoGrande", "piqa": "PIQA", "boolq": "BoolQ", "race": "RACE",
        "wikitext2": "WikiText-2", "c4": "C4", "mbpp": "MBPP", "humaneval": "HumanEval"
    }
    prompt_display = {
        "knowledge": "Knowledge", "cot": "COT", "zero_shot": "Zero-Shot", "icl": "ICL", "corpus": "Corpus"
    }
    if model_name:
